...
def test_8_chars_with_forward_slash_password_should_raise_exception():
    # given
    validator = PasswordValidator(content="c/abc@a1")
    with pytest.raises(Exception):  # then
        # when
...
def test_8_chars_with_circumflex_password_should_raise_exception():
    # given
    validator = PasswordValidator(content="c^abc@a1")
    with pytest.raises(Exception):  # then
        # when
...
def test_8_chars_with_tilde_password_should_raise_exception():
    # given
    validator = PasswordValidator(content="c~abc@a1")
    with pytest.raises(Exception):  # then
        # when
